...
| คำสั่ง | ทำอะไร |
|--------|--------|
| `/Rules` | **เริ่มที่นี่** — ดูคำสั่งและกฎทั้งหมด + อธิบายแต่ละองค์ประกอบ |
| `/FullOption_pj <งาน>` | รัน workflow ครบวงจร (before → บทบาท → ทำงาน → verify + อัปเดตความรู้) |
| `/FullOption_pj init` | โปรเจกต์ใหม่: วางโครงความรู้ + กฎพื้นฐาน |
| `/FullOption_pj status` · `commit` · `release` | ดูสถานะ · commit ตามมาตรฐาน · เตรียม release |

> รายละเอียดของแต่ละองค์ประกอบและกฎจริงของโปรเจกต์ ดูได้จากคำสั่ง `/Rules`
...
└── fulloption-pj/
    ...
    ├── commands/  FullOption_pj.md (/FullOption_pj) · Rules.md (/Rules)
